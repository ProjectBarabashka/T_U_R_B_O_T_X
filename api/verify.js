...
//     а не сырой PREMIUM_SECRET (перехват в DevTools больше не даёт Premium)
//  🔐 CRITICAL: токен содержит txHash + метод + exp, проверяется в broadcast.js
//  🐛 incVerify() вызов перенесён до tgNotify чтобы счётчик рос при ошибке TG
...
const checkIpLimit = makeRl(10, 3_600_000);
...
async function verifyLightning(paymentHash) {
  ...
  try {
    ...
    if (!data.paid)
      return {
        ok:false, method:'lightning', paid:false, expired:data.expired||false,
        error:data.expired ? 'Invoice expired' : 'Invoice not paid yet',
        expiresIn:data.expiresIn||null,
      };
    return {
      ok:true, method:'lightning', paid:true, txHash:paymentHash,
      paidAmount:`${data.amountSats?.toLocaleString()||'?'} sats`,
      amountSats:data.amountSats, confirmed:true, amountOk:true,
    };
  } catch(e) {
...
async function verifyBtc(txHash, expectedUsd) {
  ...
  return {
    ok:amountOk, method:'btc', txHash,
    paid:btcPaid.toFixed(6)+' BTC', satsPaid, confirmed,
    inMempool:!confirmed, amountOk, address:BTC_WALLET,
  };
}
...
async function verifyUsdt(txHash, expectedUsd) {
  ...
  // BUG FIX: нормализуем оба адреса для сравнения
  ...
  // BUG FIX: нормализуем адреса через base58ToHex + toLowerCase
  ...
  const toCorrectWallet = toAddr === USDT_WALLET || (toNorm && walletNorm && toNorm === walletNorm);
  ...
  return {
    ok:amountOk, method:'usdt_trc20', txHash,
    paid:usdtPaid.toFixed(2)+' USDT', usdtPaid,
    confirmed:txData.confirmed!==false, amountOk, address:USDT_WALLET,
  };
}

// BUG FIX: base58ToHex — добавлена проверка длины и правильная обработка
function base58ToHex(str) {
  ...
  // TRON адрес: 21 байт (42 hex символа) + 4 байта checksum = 25 байт (50 hex)
  // Убираем checksum (последние 8 hex = 4 байта)
...
async function tgNotify(result, ip) {
  ...
  const emoji = result.method==='btc' ? '₿' : result.method==='lightning' ? '⚡' : '💚';
...
export default async function handler(req, res) {
  ...
  if (result.ok) {
    // BUG FIX CRITICAL: не отдаём сырой PREMIUM_SECRET!
    // Генерируем подписанный HMAC токен с TTL 7 дней.
    // broadcast.js принимает и его, и старый формат (backward compat).
    const activationToken = PREMIUM_SECRET
      ? signToken({ txHash: result.txHash, method: result.method, plan: 'premium' }, PREMIUM_SECRET)
      : null;

    // BUG FIX: incVerify() вызываем ДО tgNotify (чтобы счётчик рос даже при ошибке TG)
    try { incVerify(); } catch {}
    tgNotify(result, ip).catch(()=>{});

    result.activationToken = activationToken;
    result.activatedAt     = Date.now();
    // Не возвращаем сырой секрет ни при каких условиях
