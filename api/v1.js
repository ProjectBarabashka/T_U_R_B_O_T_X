...
//     Раньше ключи созданные через ?method=keys&action=create
//     НИКОГДА не работали — Map был изолирован от auth.
//  🐛 getDynamicKeys() теперь реально используется в authenticate()
//  🆕 ?method=batch — batch broadcast через v1 API
//  🆕 X-Request-ID header в ответах для дебага
//  🆕 Последнее использование ключа трекается в lastUsed
// ══════════════════════════════════════════════════════════════
...
// BUG FIX: экспортируем для использования в authenticate()
...
// BUG FIX CRITICAL: authenticate() теперь проверяет ОБА стора
...
async function handleKeys(req) {
  ...
  if (action==='list' || req.method==='GET') {
    ...
    const dynKeys = Array.from(_dynamicKeys.values()).map(k => ({
      key:k.key.slice(0,12)+'****', tier:k.tier, name:k.name, note:k.note,
      createdAt:k.createdAt, lastUsed:k.lastUsed, requestCount:k.requestCount,
      active:k.active, source:'dynamic',
    }));
...
export default async function handler(req, res) {
  ...
  switch (method) {
    ...
    case 'fees': {
      ...
      result = { status:200, body:{ ok:true, fees:data.fees, congestion:data.congestion, history24h:data.history24h, predictions:data.predictions, mempool:data.mempool, timestamp:data.timestamp } }; break;
    }
  ...
  if (result.body && typeof result.body === 'object') {
    result.body._meta = {
      apiVersion:'v1', tier:auth.tier,
      remaining:rl.remaining, requestId,
    };
  }
