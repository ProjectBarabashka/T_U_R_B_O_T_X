// ══════════════════════════════════════════════════════════════
//  TurboTX v14.1 ★ BROADCAST ENGINE ★  —  /api/broadcast.js
//
//  BUG FIXES v14.1:
//  🔐 Принимает и старый PREMIUM_SECRET, и новый HMAC токен от verify.js
//  🐛 totalChannels: 30 → 31 (8 nodes + 23 pools включая MaraSlipstream)
//  🐛 Дублирование MARA/MaraSlipstream: MaraSlipstream НЕ вызывает MARA fallback
//     если hex доступен (избегаем двойного вызова MARA API)
//  🆕 Добавлены 3 новых пула: Bitdeer (3%), Braiins (2%), Poolin (1%)
//  🆕 Hashrate coverage: ~88% → ~94%
//  🆕 Поддержка Package Relay (Bitcoin Core 27+) через submitpackage
// ══════════════════════════════════════════════════════════════

export const config = { maxDuration: 60 };

import { CORS, getIp, sj, sleep, checkPremiumAuth } from './_shared.js';
import { incBroadcast } from './router.js';
...
const HEX_CACHE_TTL = 2 * 3_600_000;
...
function classifyError(status, body='', vercelCode='') {
  ...
  if (status===500||status===502) return 'retry_now';
  if (status===503) return 'retry_later';
  if (status===504||status===508) return 'skip';
  ...
  if (status===401||status===402||status===403||status===413||status===405) return 'permanent';
  ...
  for (let i=0; i<=tries; i++) {
    try {
      ...
      if ((cls==='retry_now'||cls==='retry_later') && i<tries) {
        await sleep(cls==='retry_now' ? 200 : 800*(i+1)); continue;
      }
...
// ─── HASHRATE TABLE Q1 2026 (UPDATED v14.1) ──────────────────
// 🆕 Добавлены Bitdeer (3%), Braiins (2%), Poolin (1%)
// Итоговый охват Premium: ~94% хешрейта
const HR = {
  ...
  MaraSlipstream:0, // дедупликация с MARA
};

const POOL_GEO = {
  Foundry:'usa', MARA:'usa', MaraSlipstream:'usa', Luxor:'usa', TxBoost:'usa', Ocean:'usa', Bitdeer:'usa',
  AntPool:'asia', ViaBTC:'asia', SpiderPool:'asia', F2Pool:'asia', CloverPool:'asia', 'BTC.com':'asia', BitFuFu:'asia',
  SBICrypto:'asia', Poolin:'asia',
  EMCDPool:'europe', Rawpool:'europe', Lincoin:'europe', Braiins:'europe',
  '2Miners':'global', mempoolAccel:'global', bitaccelerate:'global', '360btc':'global', txfaster:'global', btcspeed:'global',
};

// BUG FIX v14.1: totalChannels = 8 nodes + 23 pools = 31
const TOTAL_CHANNELS = 31;

let _lastBlockMiner = null, _lastBlockAt = 0;
const BLOCK_CACHE_TTL = 60_000;

const POOL_COINBASE_TAGS = {
  'foundry':'Foundry','foundryusa':'Foundry','antpool':'AntPool',
  'mara':'MARA','marathon':'MARA','viabtc':'ViaBTC','spiderpool':'SpiderPool',
  'f2pool':'F2Pool','luxor':'Luxor','clvpool':'CloverPool','clover':'CloverPool',
  'bitfufu':'BitFuFu','btc.com':'BTC.com','ocean':'Ocean','ocean.xyz':'Ocean',
  'emcd':'EMCDPool','sbicrypto':'SBICrypto','2miners':'2Miners','rawpool':'Rawpool',
  'bitdeer':'Bitdeer','braiins':'Braiins','poolin':'Poolin',
};
...
async function detectLastBlockMiner() {
  ...
  try {
    ...
    for (const [key, poolId] of Object.entries(POOL_COINBASE_TAGS)) {
      if (poolName.includes(key)) { _lastBlockMiner = poolId; _lastBlockAt = Date.now(); return poolId; }
    }
    return null;
  } catch { return null; }
}
...
async function updateFeeTrend() {
  ...
  try {
    ...
    const direction = _feeTrend.fastest > 0
      ...
      : newFastest > _feeTrend.fastest * 1.3 ? 'rising' : 'stable'
      : 'stable';
...
async function run(channels, feeRatioHint = 0.5, lastBlockMiner = null) {
  ...
  if (lastBlockMiner) {
    ...
    if (idx > 0) { const [boosted] = channels.splice(idx, 1); channels.unshift(boosted); }
  }
  ...
  if (!hasFreshCache && needPing.length > 0) {
    const freshPings = await Promise.allSettled(needPing.map(ch => pingChannel(ch.name).then(ms => ({name:ch.name,ms}))));
    for (const r of freshPings) if (r.status==='fulfilled') pingsMap.set(r.value.name, r.value.ms);
  }
  ...
  const WAVE_SIZE  = 12;
  const WAVE_DELAY = [0, 80, 200];
  const results    = new Array(sorted.length).fill(null);
  let okCount = 0, okHashrate = 0;
  const seenPools = new Set();

  const activeChannels = sorted.filter(ch => !isDead(ch.name) && !cbIsBlocked(ch.name) && !isCooling(ch.name) && !isNegCached(ch.name));
  const HASHRATE_STOP_TARGET = feeRatioHint < 0.4 ? 90 : feeRatioHint >= 0.8 ? 70 : 80;
  const COUNT_STOP = Math.max(3, Math.ceil(activeChannels.length * (feeRatioHint < 0.4 ? 0.90 : 0.75)));
    ...
    const launchWave = (waveChannels, waveIdx) => {
      waveChannels.forEach((ch, i) => {
        ...
        ch.call().then(r => {
          ...
          recordStat(ch.name, isOk, ms); setPing(ch.name, ms);
          ...
          if (isOk) {
            ...
            if (ch.tier==='pool' && !seenPools.has(ch.name)) {
              seenPools.add(ch.name); okHashrate += HR[ch.name] || 0;
            }
          }
          if (++finished===sorted.length) resolve();
          if (!aborted && (okHashrate >= HASHRATE_STOP_TARGET || okCount >= COUNT_STOP)) aborted = true;
        }).catch(e => {
          const ms = Date.now()-t0, isAbort = e.name==='AbortError';
          recordStat(ch.name, false, ms);
          if (!isAbort) { registerFail(ch.name); cbOnFail(ch.name); }
          results[globalIdx] = {channel:ch.name,name:ch.name,tier:ch.tier,ok:false,error:e.message,reason:isAbort?'timeout':'network_error',ms};
          if (++finished===sorted.length) resolve();
        });
      });
    };
...
async function getHex(txid) {
  ...
  SOURCES.sort((a,b) => (avgResponseMs(a.name)||9999) - (avgResponseMs(b.name)||9999));
  const TOP = SOURCES.slice(0,3), BOTTOM = SOURCES.slice(3);
  const ac = new AbortController();
...
async function analyze(txid) {
  try {
    ...
    return {
      ...
      stuckHours, isStuck72h, firstSeen,
    };
  } catch { return null; }
}
...
function txidOnlyChannels(txid) {
  const UA = getUA();
  return [
    { name:'mempoolAccel', tier:'pool', call:async()=>{ const r=await ftr('https://mempool.space/api/v1/tx-accelerator/enqueue',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'mempoolAccel','pool'); const j=await safeJson(r); return {ok:r.ok||j?.message==='Success',status:r.status,ve:ve(r)}; }},
    { name:'ViaBTC',       tier:'pool', call:async()=>{ const r=await ftr('https://viabtc.com/tools/txaccelerator/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},14000,2,'ViaBTC','pool'); const t=await safeText(r); return {ok:r.ok||t.includes('"code":0'),status:r.status,ve:ve(r)}; }},
    { name:'AntPool',      tier:'pool', call:async()=>{ const r=await ftr('https://antpool.com/txAccelerate.htm',{method:'POST',body:`txHash=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'AntPool','pool'); const t=await safeText(r); return {ok:r.ok||t.includes('success')||ok400(t,r.status),status:r.status,ve:ve(r)}; }},
    { name:'TxBoost',      tier:'pool', call:async()=>{ const r=await ftr('https://txboost.com/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'TxBoost','pool'); const t=await safeText(r); return {ok:r.ok||t.includes('success'),status:r.status,ve:ve(r)}; }},
    { name:'bitaccelerate', tier:'pool', call:async()=>{ const r=await ftr('https://www.bitaccelerate.com/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'bitaccelerate','pool'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
  ];
}
...
function freeChannels(hex, txid) {
  const UA = getUA();
  return [
    { name:'mempool.space',  tier:'node', call:async()=>{ const r=await ftr('https://mempool.space/api/tx',{method:'POST',body:hex,headers:{'Content-Type':'text/plain'}},12000,2,'mempool.space','node'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'blockstream.info',tier:'node',call:async()=>{ const r=await ftr('https://blockstream.info/api/tx',{method:'POST',body:hex,headers:{'Content-Type':'text/plain'}},12000,2,'blockstream.info','node'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'blockchair',     tier:'node', call:async()=>{ const r=await ftr('https://api.blockchair.com/bitcoin/push/transaction',{method:'POST',body:`data=${encodeURIComponent(hex)}`,headers:{'Content-Type':'application/x-www-form-urlencoded'}},12000,2,'blockchair','node'); const j=await safeJson(r); return {ok:!!(j?.data||j?.context?.code===200||ok400(JSON.stringify(j),r.status)),status:r.status,ve:ve(r)}; }},
    ...(txid ? [
      { name:'ViaBTC', tier:'pool', call:async()=>{ const r=await ftr('https://viabtc.com/tools/txaccelerator/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},14000,2,'ViaBTC','pool'); const t=await safeText(r); return {ok:r.ok||t.includes('"code":0'),status:r.status,ve:ve(r)}; }},
      { name:'mempoolAccel', tier:'pool', call:async()=>{ const r=await ftr('https://mempool.space/api/v1/tx-accelerator/enqueue',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,1,'mempoolAccel','pool'); const j=await safeJson(r); return {ok:r.ok||j?.message==='Success',status:r.status,ve:ve(r)}; }},
    ] : []),
  ];
}
...
function premiumChannels(txid, hex) {
  ...
  const nodes = hex ? [
    ...
    { name:'blockstream.info', tier:'node', call:async()=>{ const r=await ftr('https://blockstream.info/api/tx',{method:'POST',body:hex,headers:{'Content-Type':'text/plain'}},12000,2,'blockstream.info','node'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'blockchair',       tier:'node', call:async()=>{ const r=await ftr('https://api.blockchair.com/bitcoin/push/transaction',{method:'POST',body:`data=${encodeURIComponent(hex)}`,headers:{'Content-Type':'application/x-www-form-urlencoded'}},12000,2,'blockchair','node'); const j=await safeJson(r); return {ok:!!(j?.data||j?.context?.code===200||ok400(JSON.stringify(j),r.status)),status:r.status,ve:ve(r)}; }},
    { name:'blockcypher',      tier:'node', call:async()=>{ const r=await ftr('https://api.blockcypher.com/v1/btc/main/txs/push',{method:'POST',body:JSON.stringify({tx:hex}),headers:{'Content-Type':'application/json'}},12000,2,'blockcypher','node'); const j=await safeJson(r); return {ok:r.status===201||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'btcscan.org',      tier:'node', call:async()=>{ const r=await ftr('https://btcscan.org/api/tx/push',{method:'POST',body:hex,headers:{'Content-Type':'text/plain'}},10000,2,'btcscan.org','node'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'blockchain.info',  tier:'node', call:async()=>{ const r=await ftr('https://blockchain.info/pushtx',{method:'POST',body:`tx=${hex}`,headers:{'Content-Type':'application/x-www-form-urlencoded'}},12000,2,'blockchain.info','node'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'bitaps.com',       tier:'node', call:async()=>{ const r=await ftr('https://bitaps.com/api/bitcoin/push/transaction',{method:'POST',body:hex,headers:{'Content-Type':'text/plain'}},10000,2,'bitaps.com','node'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
    { name:'sochain.com',      tier:'node', call:async()=>{ const r=await ftr('https://sochain.com/api/v2/send_tx/BTC',{method:'POST',body:JSON.stringify({tx_hex:hex}),headers:{'Content-Type':'application/json'}},10000,2,'sochain.com','node'); const j=await safeJson(r); return {ok:j?.status==='success'||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
  ] : [];

  const pools = [
    { name:'Foundry',    tier:'pool', call:async()=>{ const r=await ftr('https://foundryusapool.com/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},16000,2,'Foundry','pool'); return {ok:r.ok||ok400(await safeText(r),r.status),status:r.status,ve:ve(r)}; }},
    { name:'AntPool',    tier:'pool', call:async()=>{
      try { const r=await ft('https://www.antpool.com/api/v1/tools/tx-accelerate',{method:'POST',body:JSON.stringify({txHash:txid}),headers:{'Content-Type':'application/json','User-Agent':UA,'Referer':'https://www.antpool.com/'}},12000); const j=await safeJson(r); if(r.ok||j?.code===0) return {ok:true,status:r.status}; } catch {}
      const r2=await ftr('https://antpool.com/txAccelerate.htm',{method:'POST',body:`txHash=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'AntPool','pool'); const t2=await safeText(r2); return {ok:r2.ok||t2.includes('success')||ok400(t2,r2.status),status:r2.status,ve:ve(r2)};
    }},
    // BUG FIX v14.1: MaraSlipstream НЕ делает fallback на MARA если hex есть
    // (MARA — отдельный канал, двойной вызов = зря тратим ресурсы)
    ...
    { name:'MARA',       tier:'pool', call:async()=>{ const r=await ftr('https://mara.com/api/transaction-accelerator',{method:'POST',body:JSON.stringify({txId:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},14000,2,'MARA','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'ViaBTC',     tier:'pool', call:async()=>{
      try { const r=await ft('https://viabtc.com/api/v1/btc/accelerator',{method:'POST',body:JSON.stringify({tx_id:txid}),headers:{'Content-Type':'application/json','User-Agent':UA,'Origin':'https://viabtc.com'}},14000); const j=await safeJson(r); if(r.ok||j?.code===0) return {ok:true,status:r.status}; } catch {}
      const r2=await ft('https://www.viabtc.com/tools/txaccelerator/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},14000); const t2=await safeText(r2); return {ok:r2.ok||t2.includes('"code":0'),status:r2.status,ve:ve(r2)};
    }},
    { name:'SpiderPool', tier:'pool', call:async()=>{ const r=await ftr('https://www.spiderpool.com/api/v1/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'SpiderPool','pool'); const j=await safeJson(r); return {ok:r.ok||j?.code===0||j?.success===true,status:r.status,ve:ve(r)}; }},
    { name:'F2Pool',     tier:'pool', call:async()=>{ const r=await ftr('https://www.f2pool.com/api/v2/tx/accelerate',{method:'POST',body:JSON.stringify({tx_id:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'F2Pool','pool'); const j=await safeJson(r); return {ok:r.ok||j?.code===0,status:r.status,ve:ve(r)}; }},
    { name:'Luxor',      tier:'pool', call:async()=>{ const r=await ftr('https://luxor.tech/api/accelerate',{method:'POST',body:JSON.stringify({txHash:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Luxor','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'CloverPool', tier:'pool', call:async()=>{ const r=await ftr('https://clvpool.com/accelerator',{method:'POST',body:`tx_id=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'CloverPool','pool'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
    { name:'BitFuFu',    tier:'pool', call:async()=>{ const r=await ftr('https://www.bitfufu.com/txaccelerator/submit',{method:'POST',body:JSON.stringify({txHash:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'BitFuFu','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true,status:r.status,ve:ve(r)}; }},
    { name:'BTC.com',    tier:'pool', call:async()=>{ const r=await ftr('https://btc.com/service/accelerator/boost',{method:'POST',body:JSON.stringify({tx_id:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'BTC.com','pool'); const j=await safeJson(r); return {ok:r.ok||j?.err_no===0,status:r.status,ve:ve(r)}; }},
    // 🆕 Новые пулы v14.1
    { name:'Bitdeer',    tier:'pool', call:async()=>{ const r=await ftr('https://pool.bitdeer.com/api/v1/accelerate',{method:'POST',body:JSON.stringify({txHash:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Bitdeer','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'Braiins',    tier:'pool', call:async()=>{ const r=await ftr('https://pool.braiins.com/api/v1/bitcoin/tx-accelerate',{method:'POST',body:JSON.stringify({tx_id:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Braiins','pool'); const j=await safeJson(r); return {ok:r.ok||j?.status==='ok'||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'Poolin',     tier:'pool', call:async()=>{ const r=await ftr('https://poolin.me/api/v2/tx/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Poolin','pool'); const j=await safeJson(r); return {ok:r.ok||j?.code===0||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'Ocean',      tier:'pool', call:async()=>{ const r=await ftr('https://ocean.xyz/api/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Ocean','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'mempoolAccel',tier:'pool',call:async()=>{ const r=await ftr('https://mempool.space/api/v1/tx-accelerator/enqueue',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'mempoolAccel','pool'); const j=await safeJson(r); return {ok:r.ok||j?.message==='Success',status:r.status,ve:ve(r)}; }},
    { name:'EMCDPool',   tier:'pool', call:async()=>{ const r=await ftr('https://emcd.io/api/v1/bitcoin/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'EMCDPool','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'SBICrypto',  tier:'pool', call:async()=>{ const r=await ftr('https://sbicrypto.com/api/accelerator',{method:'POST',body:JSON.stringify({tx_hash:txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'SBICrypto','pool'); const j=await safeJson(r); return {ok:r.ok||j?.status==='ok'||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'2Miners',    tier:'pool', call:async()=>{ const r=await ftr('https://2miners.com/api/v1/btc/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'2Miners','pool'); const j=await safeJson(r); return {ok:r.ok||j?.result==='ok'||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'Rawpool',    tier:'pool', call:async()=>{ const r=await ftr('https://rawpool.com/api/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Rawpool','pool'); const j=await safeJson(r); return {ok:r.ok||j?.ok===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'Lincoin',    tier:'pool', call:async()=>{ const r=await ftr('https://lincoin.com/api/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},12000,2,'Lincoin','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true||ok400(JSON.stringify(j),r.status),status:r.status,ve:ve(r)}; }},
    { name:'TxBoost',    tier:'pool', call:async()=>{ const r=await ftr('https://txboost.com/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'TxBoost','pool'); const t=await safeText(r); return {ok:r.ok||t.includes('success'),status:r.status,ve:ve(r)}; }},
    { name:'bitaccelerate',tier:'pool',call:async()=>{ const r=await ftr('https://www.bitaccelerate.com/',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'bitaccelerate','pool'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
    { name:'360btc',     tier:'pool', call:async()=>{ const r=await ftr('https://360btc.net/accelerate',{method:'POST',body:`txid=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},12000,2,'360btc','pool'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
    { name:'txfaster',   tier:'pool', call:async()=>{ const r=await ftr('https://txfaster.com/api/accelerate',{method:'POST',body:JSON.stringify({txid}),headers:{'Content-Type':'application/json','User-Agent':UA}},10000,2,'txfaster','pool'); const j=await safeJson(r); return {ok:r.ok||j?.success===true,status:r.status,ve:ve(r)}; }},
    { name:'btcspeed',   tier:'pool', call:async()=>{ const r=await ftr('https://btcspeed.org/boost',{method:'POST',body:`tx=${txid}`,headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':UA}},10000,2,'btcspeed','pool'); return {ok:r.ok,status:r.status,ve:ve(r)}; }},
  ];
...
async function bootstrapInline() {
  ...
  try {
    ...
    await Promise.allSettled(targets.map(async ({name,url}) => {
      const t0 = Date.now();
      try {
        const ac = new AbortController();
        const tm = setTimeout(()=>ac.abort(), 3000);
...
async function tg({results, txid, plan, analysis, ms, hr, ip, blocked, waveStrategy, lastBlockMiner, feeTrend}) {
  ...
  let text;
  if (blocked) {
    text=[`🛡 *TurboTX BLOCKED*`,`📋 \`${txid?.slice(0,14)||'???'}\``,`🚫 ${blocked}`,`🌐 \`${ip}\``,`🕐 ${new Date().toLocaleString('ru',{timeZone:'Europe/Moscow'})} МСК`].join('\n');
  } else {
...
export default async function handler(req, res) {
  ...
  if (effectivePlan==='premium') {
    ...
    // BUG FIX v14.1: принимаем и старый сырой секрет, и новый HMAC токен
  ...
  const summary = {
    ...
    feeRate:analysis?.feeRate??null, feeRatio:analysis?.feeRatio??null,
    needCpfp:analysis?.needCpfp??false, cpfpFeeNeeded:analysis?.cpfpFeeNeeded??0,
    rbfEnabled:analysis?.rbfEnabled??false, isStuck72h:analysis?.isStuck72h??false,
    stuckHours:analysis?.stuckHours??null, waveStrategy, lastBlockMiner:lastBlock,
    feeTrend:feeTrend?.direction??'stable',
    // BUG FIX v14.1: было 30, теперь корректно 31 (8 nodes + 23 pools)
...
async function handleBatch(req, res, body, plan, ip) {
  ...
  const batchResults = await Promise.allSettled(
    txids.map(async (txid, idx) => {
      ...
      let channels;
      if (plan==='premium') {
        channels = idx < 3 ? (hex ? premiumChannels(txid,hex) : txidOnlyChannels(txid)) : txidOnlyChannels(txid);
      } else {
      ...
      const uniqueHr = plan==='premium'
        ? [...new Set(results.filter(r=>r.ok&&r.tier==='pool').map(r=>r.name||r.channel))].reduce((s,name)=>s+(HR[name]||0), 0)
        : 0;
      return { txid, ok:okCount>0, okCount, hexFound:!!hex, hashrateReach:uniqueHr, analysis };
    })
  );

  const items = batchResults.map((r,i) => r.status==='fulfilled' ? r.value : {txid:txids[i],ok:false,error:r.reason?.message});
