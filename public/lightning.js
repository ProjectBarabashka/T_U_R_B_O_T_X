...
export default async function handler(req, res) {
  ...
  try {
    ...
    // BUG FIX: tgNotify был определён но никогда не вызывался
    tgNotify(amountSats, amountUsd, txid, ip).catch(() => {});
...
export function markInvoicePaid(paymentHash) {
  ...
  tgNotify(inv.amountSats, inv.amountUsd, null, 'webhook').catch(() => {});
  return true;
}
